...
export function DNAReportPage() {
  ...
  const stageIds = ['cover', 'stats', 'highlights', 'personality', 'story', 'advice'] as const;
  const stages = [
    <CoverStage key="cover" title={data.title} tint={tint} onNext={() => setStage(1)} />,
    <StatsStage key="stats" stats={data.stats} tint={tint} onNext={() => setStage(2)} />,
    <HighlightsStage key="highlights" highlights={data.highlights} tint={tint} onNext={() => setStage(3)} />,
    <PersonalityStage key="personality" personality={data.personality} tint={tint} onNext={() => setStage(4)} />,
    <StoryStage key="story" story={data.story} tint={tint} onNext={() => setStage(5)} />,
    <AdviceStage key="advice" advice={data.advice} tint={tint} onClose={() => navigate(-1)} />,
  ];
...
function StatsStage({ stats, tint, onNext }: { stats: DNAStats; tint: string; onNext: () => void }) {
  return (
    ...
    >
      ...
      <div style={{ fontSize: 22, fontWeight: 900, letterSpacing: '-0.01em', marginBottom: 28 }}>这季度的你</div>
...
function BigStat({
  ...
  tint,
  delay,
}: { label: string; value: string; suffix: string; tint: string; delay: number }) {
  return (
    <motion.div
...
function PersonalityStage({ personality, tint, onNext }: { personality: string[]; tint: string; onNext: () => void }) {
  return (
    ...
    >
      ...
      <div style={{ fontSize: 22, fontWeight: 900, letterSpacing: '-0.01em', marginBottom: 32 }}>AI 看到的你</div>
...
function AdviceStage({ advice, tint, onClose }: { advice: string; tint: string; onClose: () => void }) {
  return (
    ...
    >
      ...
      <div style={{ fontSize: 22, fontWeight: 900, letterSpacing: '-0.01em', marginBottom: 18 }}>下一程</div>
      <div
        style={{
          fontSize: 16,
          lineHeight: 1.7,
