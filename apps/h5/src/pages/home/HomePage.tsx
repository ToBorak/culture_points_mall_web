...
interface LayoutConfig {
  modules: { id: string; visible: boolean }[];
}
...
export function HomePage() {
  ...
  const promoBlindboxNode: ReactNode = (
  ...
  const sectionMap: Record<string, ReactNode> = {
    ...
    coach: coachNode,
    dna_entry: dnaEntryNode,
    promo_blindbox: promoBlindboxNode,
    bento: bentoNode,
  };
      ...
      <main style={{ padding: '20px 16px 60px', maxWidth: 460, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16, marginTop: 16 }}>
          {(layout?.modules ?? [])
            .filter((m) => m.visible)
            .map((m) => (
              <div key={m.id}>{sectionMap[m.id]}</div>
            ))}
        </div>
