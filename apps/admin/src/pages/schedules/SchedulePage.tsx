...
import { UserDeptTree } from '../chat/UserDeptTree';
...
export function SchedulePage() {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: 仅首屏加载一次
  useEffect(() => {
    (async () => {
      const r = await axios.get<{ items: RobotItem[] | null }>('/admin/dingtalk/robots', { headers: headers() });
      setRobots(r.data.items ?? []);
      const rm = await axios.get<{ items: RoomItem[] | null; note?: string }>('/admin/dingtalk/meeting-rooms', {
        headers: headers(),
      });
      setRooms(rm.data.items ?? []);
      setRoomsNote(rm.data.note ?? '');
      await loadList();
  ...
  const submit = async () => {
    ...
    try {
      ...
      const { data } = await axios.post<{ status: string; resultNote: string }>('/admin/schedules', body, {
        headers: headers(),
      });
    ...
    <div style={{ padding: 24 }}>
      ...
      <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap' }}>
        ...
        >
          ...
          <div style={{ fontSize: 12, color: 'var(--cpm-text-tertiary)' }}>开始</div>
          <input style={inputStyle} type="datetime-local" value={start} onChange={(e) => setStart(e.target.value)} />
          <div style={{ fontSize: 12, color: 'var(--cpm-text-tertiary)' }}>结束</div>
          ...
          <div style={{ fontSize: 12, color: 'var(--cpm-text-tertiary)' }}>会议室（可不选）</div>
          <select style={inputStyle} value={roomId} onChange={(e) => setRoomId(e.target.value)}>
            ...
            {rooms.map((rm) => (
              <option key={rm.roomId} value={rm.roomId}>
                {`${rm.roomName}${rm.capacity ? `（${rm.capacity}人）` : ''}`}
              </option>
            ))}
          </select>
          {roomId && (
            <div style={{ fontSize: 12, color: 'var(--cpm-text-tertiary)', marginTop: -6, marginBottom: 12 }}>
              需勾选下方「建钉钉日历」并选 ≥1 名参与员工，会议室才会真正预定。
            </div>
          )}
          {rooms.length === 0 && roomsNote && (
            <div style={{ fontSize: 12, color: 'var(--cpm-text-tertiary)', marginTop: -6, marginBottom: 12 }}>
              {roomsNote}
            </div>
          )}
          ...
          <div style={{ marginBottom: 10 }}>
            ...
            <UserDeptTree value={attendees} onChange={setAttendees} />
          </div>
          <div style={{ marginBottom: 10 }}>
            ...
            {robots.map((r) => (
              <label
                key={r.id}
                style={{ display: 'inline-flex', alignItems: 'center', gap: 4, marginRight: 12, fontSize: 13 }}
              >
                <input
                  type="checkbox"
                  checked={groups.includes(r.id)}
                  onChange={() => toggle(groups, r.id, setGroups)}
                />
                {r.name}
              </label>
            ))}
          ...
          <div style={{ marginBottom: 14, fontSize: 13 }}>
            <label style={{ marginRight: 16 }}>
              <input type="checkbox" checked={pushCalendar} onChange={(e) => setPushCalendar(e.target.checked)} />{' '}
              建钉钉日历
            </label>
            <label>
          ...
          <Button tone="primary" size="md" onClick={submit} disabled={busy}>
            {busy ? '发布中...' : '发布'}
          </Button>
          {msg && (
            <div style={{ marginTop: 12, fontSize: 13, color: 'var(--cpm-text-secondary)', wordBreak: 'break-all' }}>
              {msg}
            </div>
          )}
        </div>
          ...
          {list.length === 0 ? (
            ...
            list.map((s) => (
              ...
              >
                ...
                <div style={{ fontSize: 12, color: 'var(--cpm-text-tertiary)', marginTop: 4 }}>
                  状态：{s.status} · {s.resultNote}
                </div>
              </div>
            ))
          )}
